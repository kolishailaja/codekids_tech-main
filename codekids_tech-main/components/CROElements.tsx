...
import { CheckCircle, Star, Users, Award, Clock, Zap, Shield, TrendingUp, Phone, MessageCircle } from 'lucide-react';
...
export function TrustSignal({ count, label, icon, highlight }: TrustSignalProps) {
  return (
    <motion.div
      ...
      className={`flex flex-col items-center p-4 rounded-lg ${highlight ? 'bg-primary/10 border-2 border-primary' : 'bg-secondary/50'}`}
    >
      <div className={`${highlight ? 'text-primary' : 'text-foreground/70'} mb-2`}>
        {icon}
      </div>
      {count && (
        <motion.div
      ...
      <p className={`text-sm mt-1 ${highlight ? 'text-foreground font-semibold' : 'text-muted-foreground'}`}>
        {label}
      </p>
    </motion.div>
  );
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <p className={`font-semibold ${highlight ? 'text-orange-700 dark:text-orange-300' : 'text-foreground'}`}>
            {message}
          </p>
          {deadline && (
            <p className="text-sm text-muted-foreground mt-1">⏰ {deadline}</p>
          )}
        </div>
      </div>
    </motion.div>
...
export function GuaranteeBadge({ title, description, icon }: GuaranteeBadgeProps) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="glass-card p-6 rounded-xl border-2 border-primary/20 hover:border-primary/40 transition-all"
    >
