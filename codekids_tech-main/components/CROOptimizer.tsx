    ...
    <>
      ...
      {showUrgencyBanner && showUrgencyBannerState && (
        <motion.div
          ...
          animate={{ y: 0 }}
          exit={{ y: -100 }}
          className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-primary via-purple-600 to-primary text-white px-4 py-3 shadow-lg"
        >
          <div className="max-w-7xl mx-auto flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <span className="hidden md:inline">⏰ Enroll Today - Get 20% Off + Free Demo Session!</span>
            </div>
            <div className="flex items-center gap-2">
              <motion.button
                ...
                className="px-4 py-2 bg-white text-primary rounded-full font-semibold text-sm hover:shadow-lg transition-shadow flex items-center gap-2"
              >
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={handleCall}
                    className="flex-1 px-6 py-3 bg-primary text-white rounded-full font-semibold hover:shadow-lg transition-shadow flex items-center justify-center gap-2"
                  >
      ...
      <AnimatePresence>
        {showTrustSignals && showTrustSignalsState && (
          <motion.div
            ...
            transition={{ delay: 2 }}
            className="fixed bottom-24 left-4 right-4 md:left-auto md:right-6 md:w-auto z-40 hidden md:block"
          >
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 p-4 backdrop-blur-lg relative">
              {/* Close Button */}
              <button
                onClick={handleHideTrustSignals}
                className="absolute -top-2 -right-2 h-6 w-6 rounded-full bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 flex items-center justify-center transition-colors z-10"
      ...
      <AnimatePresence>
        {showTrustSignals && showTrustSignalsState && (
          <motion.div
            ...
            exit={{ y: 100, opacity: 0 }}
            transition={{ delay: 2 }}
            className="fixed bottom-24 left-0 right-0 z-40 block md:hidden px-4"
          >
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 p-3 relative">
              ...
              <div className="flex items-center justify-around gap-2">
                <div className="flex flex-col items-center gap-1">
                  ...
                  <p className="text-xs font-bold text-gray-900 dark:text-white">1,250+</p>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400">Students</p>
                </div>
                <div className="flex flex-col items-center gap-1">
                  <Star className="h-5 w-5 text-yellow-500 fill-current" />
                  <p className="text-xs font-bold text-gray-900 dark:text-white">4.9/5</p>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400">Rating</p>
                </div>
                <div className="flex flex-col items-center gap-1">
                  <Award className="h-5 w-5 text-blue-500" />
                  <p className="text-xs font-bold text-gray-900 dark:text-white">20+</p>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400">Schools</p>
                </div>
                <div className="flex flex-col items-center gap-1">
                  <TrendingUp className="h-5 w-5 text-green-500" />
                  <p className="text-xs font-bold text-gray-900 dark:text-white">90%</p>
